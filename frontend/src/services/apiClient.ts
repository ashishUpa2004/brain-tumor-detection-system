  ...
  (error: AxiosError<ApiErrorResponse>) => {
    ...
    if (error.response) {
      ...
      if (error.response.data) {
        // Backend format: { "status": "error", "message": "..." } or { "detail": "..." }
      ...
      switch (status) {
        case 401:
          ...
          // For login failures, don't redirect, just show error
...
export const login = async (credentials: LoginFormData): Promise<AuthResponse> => {
  ...
  // Backend returns: { "status": "success", "data": { "access_token": "...", "token_type": "bearer", "user": {...} } }
...
export const predict = async (formData: FormData): Promise<PredictionApiResponse> => {
  ...
  if (response.data.status === 'success' && response.data.data) {
    ...
    return {
      ...
      gradCamUrl: data.heatmap_url || data.grad_cam_url,
